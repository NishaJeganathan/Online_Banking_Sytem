...
const Account = {
  ...
  async getAllTransactions(bankId, acc_no) {
    ...
    const [sent] = await db.query(
      "SELECT transaction_id,recv_bank,recv_acc_no,amount,timestamp,status FROM transactions WHERE acc_no=? and sender_bank=? ORDER BY timestamp DESC",
      [acc_no, bankId]
    );

    const [received] = await db.query(
      "SELECT transaction_id,sender_bank,acc_no AS sender_account,amount,timestamp,status FROM transactions WHERE recv_acc_no=? and recv_bank=? ORDER BY timestamp DESC",
      [acc_no, bankId]
    );
